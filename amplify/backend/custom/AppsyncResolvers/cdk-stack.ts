...
export class cdkStack extends cdk.Stack {
   ...
   * @param dependencies
   */
  private createQueryResolvers(
    dependencies: AmplifyDependentResourcesAttributes
  ) {
    for (let [key, value] of queryMap) {
      const requestTemplate = fs.readFileSync(
        path.join(__dirname, '..', 'resolvers', 'query', value.requestTemplate),
        'utf-8'
      );
   ...
   * @param dependencies
   */
  private createMutationResolvers(
    dependencies: AmplifyDependentResourcesAttributes
  ) {
